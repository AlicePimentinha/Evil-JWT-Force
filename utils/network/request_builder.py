"""
RequestBuilder - Módulo avançado para construção de requisições HTTP
"""
...
# Integração com ProxyRotator e User-Agent rotator se disponíveis
try:
...
USER_AGENTS = [
    # Lista reduzida, pode ser expandida conforme necessário
...
class RequestBuilder:
    """Classe avançada para construção de requisições HTTP com suporte a proxies, rotação de user-agent, autenticação e manipulação de cookies."""
...
def build_form_payload(fields: Dict[str, Any]) -> Dict[str, Any]:
    """Constrói payload para envio em multipart/form-data ou application/x-www-form-urlencoded"""
    return fields

def build_files_payload(file_paths: List[str]) -> Dict[str, Any]:
    """Constrói payload para upload de arquivos"""
    files = {}
...
def close_files(files: Dict[str, Any]):
    """Fecha arquivos abertos em payloads"""
